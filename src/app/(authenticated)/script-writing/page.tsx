...
const downloadAsDocx = async (title: string, content: string) => {
  ...
  const blob = new Blob([buffer], {
    type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.script',
  })
...
export default function DocumentWritingPage() {
  ...
  const handleGenerate = async () => {
    try {
      ...
      const prompt = `Craft an exceptional and innovative ${tone} script about ${topic} that captivates and resonates deeply with readers. Masterfully weave these key phrases throughout the narrative: ${keywords}. Create a compelling piece of approximately ${
        length >= 100 ? length : 200
      } words that follows a clear, engaging structure with strategic headings and subheadings. The script should begin with a powerful hook, develop through well-crafted sections that build momentum, and conclude with an irresistible call-to-action that inspires immediate response. Focus on delivering unique insights, emotional connection, and actionable value while maintaining impeccable flow and natural keyword integration. The final piece should be memorable, shareable, and position you as an authoritative voice on ${topic}.`
      ...
      const script = await createArticle.mutateAsync({
        data: {
          title: topic,
          topic,
          keywords,
      ...
      enqueueSnackbar('Script generated successfully!', {
        variant: 'success',
      })
      handleSelectArticle(script)
      refetch()
    } catch (error) {
      enqueueSnackbar('Failed to generate script', { variant: 'error' })
    } finally {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      enqueueSnackbar('Failed to update script', { variant: 'error' })
    }
  }
  ...
  const handleDownload = async (format: string) => {
    ...
    try {
      ...
      enqueueSnackbar(`Script downloaded as ${format}`, {
        variant: 'success',
      })
    } catch (error) {
      enqueueSnackbar('Failed to download script', { variant: 'error' })
    }
  }

  const handleSelectArticle = (script: any) => {
    setSelectedArticle(script.id)
    setContent(formatContent(script.content))
  }

  useEffect(() => {
    if (articleId && articles) {
      const script = articles.find(a => a.id === articleId)
      if (script) {
        handleSelectArticle(script)
      }
    }
    ...
    <>
      <PageLayout layout="narrow">
        <Title level={2} style={{ color: '#ffffff' }}>
          <FileTextOutlined /> AI Script Writer
        </Title>
            ...
            <Row gutter={16}>
              <Col span={12}>
                <input
                  ...
                  className="w-full bg-gray-700/50 text-white border border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
                  placeholder="Enter script length (Default: 200)"
                  min={1}
                />
              </Col>
              <Col span={12}>
        ...
        <div className="flex flex-col md:flex-row gap-4 mt-10">
          <div className="md:w-1/3 w-full">
            <Title level={3} style={{ color: '#ffffff' }}>
              <HistoryOutlined /> Script History
            </Title>
            <Card
              style={{
              ...
              {articles?.map(script => (
                ...
                >
                  <Text strong style={{ color: '#ffffff', fontSize: '1.2rem' }}>
                    {script.title.slice(0, 50)}
                  </Text>
                  <br />
                  <Text type="secondary" style={{ color: '#ffffff80' }}>
                    {dayjs(script.createdAt).format('MMM D, YYYY')}
                  </Text>
                </div>
              ))}
              {articles?.length === 0 && (
            ...
            <Card
              ...
              extra={
                <Space style={{ color: '#ffffff' }}>
                  ...
                  <Button
                    onClick={() => {
                      const script = articles?.find(
                        a => a.id === selectedArticle,
                      )
                      if (script) {
                        downloadAsDocx(script.title, content)
                      }
                    }}
