...
export default function DocumentWritingPage() {
  ...
  const handleGenerate = async () => {
    try {
      ...
      const prompt = `Create a standout, ${tone} document on ${topic}. Incorporate these essential keywords effectively throughout the text: ${keywords}. Aim for a document length of ${
        length >= 100 ? length : 200
      } words, ensuring it is well-organized, captivating, and includes headings, subheadings, and a compelling call-to-action. Prioritize clarity, keyword integration, and providing substantial value to the intended audience.`
      ...
      const document = await createArticle.mutateAsync({
        data: {
          title: topic,
          topic,
          keywords,
  ...
  const handleDownload = async (format: string) => {
    ...
    try {
      ...
      enqueueSnackbar(`Document downloaded as ${format}`, {
        variant: 'success',
      })
    } catch (error) {
  ...
  const handleSelectArticle = (document: any) => {
    setSelectedArticle(document.id)
    setContent(formatContent(document.content))
  }

  useEffect(() => {
    if (articleId && articles) {
      const document = articles.find(a => a.id === articleId)
      if (document) {
        handleSelectArticle(document)
      }
    }
    ...
    <>
      <PageLayout layout="narrow">
        <Title level={2} style={{ color: '#ffffff' }}>
          <FileTextOutlined /> AI Document Writer
        </Title>
            ...
            <Row gutter={16}>
              <Col span={12}>
                <input
                  ...
                  className="w-full bg-gray-700/50 text-white border border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
                  placeholder="Enter document length (Default: 200)"
                  min={1}
                />
              </Col>
              <Col span={12}>
        ...
        <div className="flex flex-col md:flex-row gap-4 mt-10">
          <div className="md:w-1/3 w-full">
            <Title level={3} style={{ color: '#ffffff' }}>
              <HistoryOutlined /> Document History
            </Title>
            <Card
              style={{
            ...
            <Card
              ...
              extra={
                <Space style={{ color: '#ffffff' }}>
                  ...
                  <Button
                    onClick={() => {
                      const document = articles?.find(
                        a => a.id === selectedArticle,
                      )
                      if (document) {
                        downloadAsDocx(document.title, content)
                      }
                    }}
